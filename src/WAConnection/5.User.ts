...
export class WAConnection extends Base {
    ...
    async getStatus(jid?: string) {
        const status: { status: string } = await this.query({
            json: ['query', 'Status', jid || this.user.jid],
    ...
    async getContacts() {
        ...
        const response = await this.query({
            json,
            binaryTags: [WAMetric.queryContact, WAFlag.ignore],
            expect200: true,
    ...
    async getStories() {
        ...
        const response = await this.query({
            json,
            binaryTags: [WAMetric.queryStatus, WAFlag.ignore],
            expect200: true,
            requiresPhoneConnection: true
        }) as WANode
